function [Zk_all, MaximumResponses, ground_truth] = tracker(video_path, ...
...
% If large target, lower the resolution, we don't need that much detail.
 ...
 GT = ground_truth ;   %  ground_truth is lower-left corner of bounding box
...
posDeltas        = zeros(numel(img_files), 8) ;
...
%%   End pre-allocation
Zk = pos1 ;    % Initialize Kalman est and Measurement to GT at frame 1
Xk = Zk ;
...
MaximumResponses(frame-1,:) = HOGresponseMax;
 ...
 if horiz_delta > size(zf,2) / 2  %same for horizontal axisZkZk
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  posDeltas saves measurements in this order-
%  x,y velocity change | RMS velocity change | x,y Kalman est | maxHog |
%  x,y GT
    if frame > 1     % Compute velocity (pixel_k - pixel_k-1) x,y,rms
        posRMS = sqrt( (Zk(1)-Zkprior(1))^2 + (Zk(2)-Zkprior(2))^2) ;
        posDeltas(frame,:) = [(Zk - Zkprior) posRMS Zk ...
            HOGresponseMax GT(frame,1:2) ] ;
        kalmanErr = sqrt( (Zk(1)-GT(frame,1))^2 + (Zk(2)-GT(frame,2))^2) ;
        hogErr = sqrt( (Zk(1)-GT(1))^2 + (Zk(2)-GT(2))^2) ;
    end
...
sv2 = ([.1 .1]').^2 ;          % standard deviation of vk (measurement noise)
...
Xk    =   [Zk 0 0]' ;         % initial object location
Pk    =   diag(P) ;      
...
% Zk = Xk(1:2)' ;
if frame >= 2
    R = eye(2);      % R is now controlled by function below
 %    HOGvariance function enables use of Kalman filter...and allows
 %    modifications to R (measurement variance)
% sv2 = HOGvariance(response(:), HOGresponseMax, ...
%     frame, Xk, Zk, sv2, posDeltas);
end
...
% Zk = Xk(1:2)' ; %  +nullFactor(1:2) ;
 boundingBoxes(GT(frame,:), frame, GT_Box, box, padding, target_sz, Xk, ...
     nullFactor) ;
